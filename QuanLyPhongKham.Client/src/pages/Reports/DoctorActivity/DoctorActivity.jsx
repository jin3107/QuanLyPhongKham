...
export default function DoctorActivity() {
  ...
  const handleStatistic = () => {
    ...
    const filtered = PHIEU_KHAM.filter(
      (p) => p.NgayKham >= s && p.NgayKham <= e,
    );
  ...
  const maxCount = rows.length
    ? Math.max(...rows.map((r) => r.SoPhieuKham))
    : 1;
        ...
        <div className="count-cell">
          <div className="count-bar-wrap">
            <div
              className="count-bar"
              style={{
                width: maxCount > 0 ? `${(v / maxCount) * 100}%` : "0%",
              }}
            />
          </div>
      ...
      <div className="ds-body">
        ...
        <div className="filter-card">
          <div className="section-label">
            <BarChartOutlined /> Chọn khoảng thời gian thống kê
          </div>
        ...
        {searched && summary && (
          <div className="summary-chips">
            <div className="chip">
              <TeamOutlined /> <b>{summary.totalBS}</b> bác sĩ
            </div>
            <div className="chip">
              <BarChartOutlined /> Tổng <b>{summary.total}</b> phiếu khám
            </div>
          </div>
        )}
        ...
        {searched && (
          <div className="table-wrap">
            <div className="table-title">
              <BarChartOutlined /> Thống kê hoạt động bác sĩ
            </div>
            <Table
              columns={columns}
              dataSource={rows}
